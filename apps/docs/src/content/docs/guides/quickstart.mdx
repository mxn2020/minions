...
<Tabs syncKey="lang">
  ...
  const agent = await minions.create('agent', { title: 'Agent' });
  const skill = await minions.create('note', { title: 'Skill' });
  ...
  # Create an agent and a skill
  agent = await minions.create("agent", {"title": "Agent"})
  skill = await minions.create("note", {"title": "Skill"})
...
- Add [middleware & storage hooks](/guides/middleware/) for logging, auth, and more
