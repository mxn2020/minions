...
```python
from minions import Minions, YamlFileStorageAdapter

storage = await YamlFileStorageAdapter.create("./data/minions")
minions = Minions(storage=storage)
```
...
> [!TIP]
> The `toYaml()` and `parseYaml()` utilities are also exported from `minions-sdk/node` for standalone YAML conversion outside of the storage layer.
